...
if [ -n "${BASH_VERSION:-}" ]; then
  _fleet_complete() {
    ...
    if [ "$COMP_CWORD" -eq 1 ]; then
      COMPREPLY=($(compgen -W "setup start attach send broadcast log run doctor respawn boot login caffeinate decaffeinate remote remote-ssh keys remote-install tray hosts config update grid spread status ls stop restart help" -- "$cur"))
